...
local function waitExact(t)
    local s = os.clock()
    repeat task.wait() until os.clock() - s >= t
end

local function updateHRP(c)
    local h = c:WaitForChild("HumanoidRootPart",5)
    if h then _G.AutoSlotHRP = h end
end
...
local function savePos()
    if _G.AutoSlotHRP then
        _G.ReturnPos = _G.AutoSlotHRP.CFrame
    end
end

local function loadPos()
    if _G.AutoSlotHRP and _G.ReturnPos then
        _G.AutoSlotHRP.CFrame = _G.ReturnPos
        _G.ReturnPos = nil
    end
end
        ...
        while true do
            ...
            if h and t and _G.AutoSlotHRP then
                ...
                pcall(function()
                    ...
                    waitExact(0.5)
                    ReplicatedStorage:WaitForChild("GrabEvents"):WaitForChild("SetNetworkOwner"):FireServer(h,CFrame.new(h.Position))
                    waitExact(0.8)
