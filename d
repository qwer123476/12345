...
local getTab = Window:MakeTab({
    Name = "알림",
    Icon = "rbxassetid://649498361",
    PremiumOnly = false
})
...
local DDTab = Window:MakeTab({
    Name = "티배깅",
    Icon = "rbxassetid://0",
    PremiumOnly = false
})

local FDTab = Window:MakeTab({
    Name = "친구 보호",
    Icon = "rbxassetid://6026568198",
    PremiumOnly = false
})
...
local antiTab = Window:MakeTab({
    Name = "안전",
    Icon = "rbxassetid://6034831332",
    PremiumOnly = false
})

local ListTab = Window:MakeTab({
    Name = "명단",
    Icon = "rbxassetid://3572491301",
    PremiumOnly = false
})
...
local function autoGradient(text, target, startColor, endColor)
    ...
    return string.gsub(
        text,
        string.gsub(target, "([%(%)%.%%%+%-%*%?%[%]%^%$])", "%%%1"),
        result
    )
end

local function applyRichTextAndEffects(gui)
